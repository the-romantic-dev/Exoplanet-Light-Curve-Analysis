...
void occultquad_tmid(double *t, double p, double ar, double P, double i, double gamma1, double gamma2, double e, double longPericenter, double *tmid, double n, double *F);
...
void phasecurve_old(double *t, double *C, double fpfs, double rprs, double ars, double P, double inc, double gamma1, double gamma2, double e, double longPericenter, double tmid, double n, double *F)
{
	// transit 
...
void occultquad(double *t, double p, double ar, double P, double i, double gamma1, double gamma2, double e, double longPericenter, double tmid, double n, double *F)
{
	...
	for (j=0;j<Npoints;j++)
	{
		...
		if (z>=(1+p) || p==0 || fabs(phi[j])>(p+1.0)*invar*0.5*invPi) { // Case 1
			F[j] = 11; // F[j] != 1 when planet is not in sight
		}
...
void occultquad_tmid(double *t, double p, double ar, double P, double i, double gamma1, double gamma2, double e, double longPericenter, double *tmid, double n, double *F)
{
	...
	for (ii=0; ii<Npoints; ii++)
	{
		...
		// looping error perhaps comes from somewhere in here
		tp = tmid[ii]+P*sqrtee*0.5*invPi*(e1*sin(f1)/(1.0+e1*cos(f1))-2.0/sqrtee*atan( (sqrtee*tan(0.5*f1))/(1.0+e1) ));
		m = 2.0*pi*invP*(ti-tp);
		f = kepler_opt(m,epoverm,e1);
		radius = ar*(1.0 - e1*e1)/(1.0 + e1*cos(f));
		Z[ii] = radius*sqrt(1.0-(sin(i*pi*inv180)*sin(longPericenter*pi*inv180+f))*(sin(i*pi*inv180)*sin(longPericenter*pi*inv180+f))); //Eric Agol's code inspired		
